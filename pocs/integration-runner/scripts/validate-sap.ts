...
const spec = {
  ...
  linkPipelines: buildLinkPipelines(yaml),
  graphClient: createStubGraphClient(),
} as Parameters<typeof integrate>[0];
integrate(spec);
console.log(`[validate] OK: ${spec.pipelines.length} pipelines, ${spec.linkPipelines?.length ?? 0} links`);
