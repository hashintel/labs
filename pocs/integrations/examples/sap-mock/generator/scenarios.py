"""Scenario injection (inventory / production / supplier events). Ported from hashintel/SAP_Mock_Data (Inject Scenarios notebook); logic verbatim, Spark shell replaced with pandas + Delta warehouse io."""
import pandas as pd
import numpy as np
import random
...
from common import param, widget, seed_all
    ...
    if not wh.exists(table_name):
        ...
        for col, dtype in zip(target.columns, target.dtypes):
            if col in df.columns:
                ...
                # Defaults, mirroring the notebook's per-Delta-type F.lit choices.
                # Read-back string columns may be object or arrow-backed StringDtype.
...
def generate(wh):
    ...
    if inventory_changes or supplier_changes or production_changes:
        ...
        write_deltalake(str(wh.root / metadata_table), df_metadata.reset_index(drop=True), mode="overwrite", schema_mode="overwrite")
