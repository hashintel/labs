...
impl Properties {
    ...
    /// TODO(haze): Audit this (Can a `serde_json::Value` be anything else?)
    ...
    #[allow(clippy::cognitive_complexity, clippy::too_many_lines)] // TODO(haze): split up
    #[must_use]
    pub fn topology_config(&self) -> Option<Config> {
        ...
        if let Some(serde_json::Value::Object(topology_props)) = self.0.get("topology") {
            ...
            */
            ...
            {
                use crate::distance::functions::{conway, euclidean, euclidean_squared, manhattan};
                config.distance_function = Box::new(match &**distance_func_str {
                    "manhattan" => manhattan,
                    "euclidean" => euclidean,
                    "euclidean_squared" => euclidean_squared,
                    // "conway" | "chebyshev" => conway,
                    // default is already conway, TODO(haze) nicer alternative
                    _ => conway,
                });
            }
