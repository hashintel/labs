// Adapted from https://github.com/microsoft/vscode/blob/a1de2a783afd8c9e64d3ddbf517df727f9f6cdef/src/vs/editor/contrib/find/replacePattern.ts
...
export class ReplacePattern {
  ...
  public buildReplaceString(
    ...
    let result = "";
    for (let idx = 0, len = this._state.pieces.length; idx < len; idx++) {
      const piece = this._state.pieces[idx];
...
export function parseReplaceString(replaceString: string): ReplacePattern {
  ...
  for (let idx = 0, len = replaceString.length; idx < len; idx++) {
    const chCode = replaceString.charCodeAt(idx);
    ...
    if (chCode === CharCode.Backslash) {
      ...
      const nextChCode = replaceString.charCodeAt(idx);
    ...
    if (chCode === CharCode.DollarSign) {
      ...
      if (nextChCode === CharCode.Digit0 || nextChCode === CharCode.Ampersand) {
        // $& and $0 => inserts the matched substring.
        result.emitUnchanged(idx - 1);
        result.emitMatchIndex(0, idx + 1, caseOps);
        caseOps.length = 0;
        continue;
      }
      ...
      if (CharCode.Digit1 <= nextChCode && nextChCode <= CharCode.Digit9) {
        ...
        if (idx + 1 < len) {
          const nextNextChCode = replaceString.charCodeAt(idx + 1);
          if (
            ...
            result.emitUnchanged(idx - 2);
            result.emitMatchIndex(matchIndex, idx + 1, caseOps);
            caseOps.length = 0;
            continue;
          }
        }
