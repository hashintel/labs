      ...
      await Promise.all(
        runs.map(async (run) => {
          if (!run.plots && run.analysisLink) {
            const analysis = await historicCloudExperimentProvider.getAnalysis(
              experiment,
              run
            );
  ...
  if (run.experimentId && !run.stepsCount && run.stepsLink) {
    ...
    if (experiment) {
      ...
      try {
        ...
        clearTimeout(timeout);
        dispatch(
          setSimulationStatus({ simId: run.simulationRunId, status: "errored" })
        );
        console.error(err);
      }
    }
 ...
 */
export const setViewerSpeed = (speed: number | "live"): SimulatorThunk => (
  dispatch,
  getState
) => {
  const state = getState();
...
export const earlyStopSimulation = (
  simId: string,
  message: unknown
): SimulatorThunk => (dispatch, getState) => {
