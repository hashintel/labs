...
use hashintel_core::{
    ...
    prelude::{
        AgentState, Context, DatasetMap, DefaultRuntime, OutboundMessage, Properties,
        SimulationResult, SimulationState, Vec3,
    },
    sim,
};
use rand::prelude::*;
...
fn timed_pi(run_count: usize) -> (Vec<Rc<SimulationState>>, Vec<std::time::Duration>) {
    ...
    */
    ...
    // Pull in Context and our custom SimulationError bound Result into our code:
    /*
    ...
    */
    ...
    let spawn_behavior = NamedBehavior::new("spawn_samples", spawn_fn, vec![""]);
    ...
    */
    ...
    let leader = LeaderAgent {
        ...
        neighbor_points: 0,
        pi_estimate: 0.0,
        behaviors: vec!["spawn_samples".to_string(), "estimate_pi".to_string()],
        position: pos,
        agent_name: "leader".to_string(),
        search_radius: 1.0_f64,
    };

    let initial_state: Vec<AgentState> = vec![serde_json::to_value(&leader).unwrap().into()];
    ...
    let mut sim = Box::pin(sim::create_simulation(
        initial_state,
        ...
        .unwrap(),
        DatasetMap::new(),
        vec![spawn_behavior, estimate_pi_behavior],
        vec![],
        sim_config,
        DefaultRuntime::new(),
    ));
...
fn main() {
    let (results, times) = timed_pi(100);

    let set_times = vec![
        1, 2, 3, 4, 5, 10, 20, 30, 50, 80, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000,
    ];
    let len = &times.len();
