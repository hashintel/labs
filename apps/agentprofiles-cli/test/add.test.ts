...
describe('addCommand symlink preservation', () => {
  ...
  it('creates relative default instruction links for directory-strategy profiles cloned from _base', async () => {
    const config = new ConfigManager();
    await config.ensureConfigDir();
    ...
    const baseDir = path.join(contentDir, 'amp', BASE_PROFILE_SLUG);
    await fs.mkdir(baseDir, { recursive: true });
    await fs.writeFile(
      path.join(baseDir, 'meta.json'),
      JSON.stringify(
    ...
    await fs.symlink('../../_agents/AGENTS.md', path.join(baseDir, 'AGENTS.md'));

    const { addCommand } = await import('../src/commands/add.js');
    await addCommand('amp', 'work');

    const profileInstruction = path.join(contentDir, 'amp', 'work', 'AGENTS.md');
    expect((await fs.lstat(profileInstruction)).isSymbolicLink()).toBe(true);
    expect(await fs.readlink(profileInstruction)).toBe('../_base/AGENTS.md');
  });
